---
title: "Division Resurgence Inventory Bug Fixed and Phase 3 News"
publishDate: "2026-05-23T23:59:59Z"
author: "RapidF5"
featuredImage: "/images/news/inventory-wipe-bug-fixed.jpg"
imageAlt: "A Division Resurgence Agent looking at a glitching orange holographic smartwatch interface on a snowy New York street, representing the inventory out-of-bounds bug error."
description: "Ubisoft fixes the disappearing inventory index error, Strawberry Milkshake underperforms, and Phase 3 approaches. Here's your 48-hour Resurgence brief."
tags: ["News", "Patch Notes", "Phase 2", "Strawberry Milkshake", "Aegis", "Dark Zone", "Bugs"]
shareTitle: "The Inventory Bug is Dead"
shareText: "The disappearing loot bug is fixed. Here's the full breakdown plus why the Strawberry Milkshake is the new Pestilence."

faqs:
  - question: "Did Ubisoft fix the disappearing inventory bug in Division Resurgence?"
    answer: "Yes, a May 21 server-side patch addressed the 'index out of bounds' error causing inventory wipes. In non-developer terms: the game's inventory system was trying to access a storage slot that didn't exist, and your god-rolled Lady Death fell into the digital void. The fix stops new items from disappearing. Compensation for previously lost items is being handled via support tickets, which means 'send a ticket and pray.'"
  - question: "Will I get my lost items back from the inventory bug?"
    answer: "The blanket compensation was 500k Basic Credits and 750 Phoenix Credits for all affected players. For specific lost items, you need to submit a support ticket to Ubisoft customer service. Community reports on success rates are mixed. Some players got items restored, others got a form response. If you lost a max-rolled Warlord, our condolences. And our suggestion to submit that ticket immediately."
  - question: "Is it safe to play Division Resurgence now or will my items disappear again?"
    answer: "The root cause has been patched. The 'index out of bounds' error has been fixed at the server level, meaning the specific bug that caused items to vanish in transit between your inventory and the database is no longer active. That said, this is a live service game. New bugs can always appear. But the specific inventory-eating demon has been exorcised."
---

## Ubisoft Names the Boss: Inventory-Wipe Bug Was an Index-Out-of-Bounds Error
**Plus Everything Else From a Suspiciously Quiet 48 Hours**

It’s been a weird couple of days in Manhattan. While Patch 1.2.1 tried to put a bandage on the chaos of Phase 2’s launch, it took a quiet server-side patch 24 hours later to finally kill the final boss of Season 1: **the disappearing inventory bug.** 

Meanwhile, the community has gotten its hands on the new [exotic LMG](/database/exotic-weapons/), Aegis is sparking fierce [build debates](/guides/which-specialization-to-pick/), and the Dark Zone PvP meta is as salty as ever. Here is your unfiltered 48-hour news brief.

---

## 🔧 LEAD STORY: The Inventory Bug Was an "Index-Out-of-Bounds" Error

The symptom was terrifying: log in, open your inventory, and watch your god-rolled Lady Death simply cease to exist. 

The cause, according to an [official update from the dev team](https://reddit.com/r/thedivisionresurgence/comments/1tjto41/item_disappearing_fix_update/), was an **Index-Out-of-Bounds** error. In non-developer terms: the inventory bag was trying to access a slot that didn't exist in the database, dropping your hard-earned items into the digital void in transit. 

On **May 21**, Ubisoft pushed a server-side patch to address the root cause:
- Fixed the inventory bag not returning all item slots to the client.
- Fixed the inventory bag not saving all items into the database.
- Fixed items randomly disappearing.

### The Catch: No Retroactive God Rolls
While the bleed has stopped, the healing hasn't started for everyone. The 500k Basic Credit and 750 Phoenix Credit blanket payout was nice, but **there is no retroactive compensation for specific lost items.** 

If you lost a max-rolled Warlord last week, you are still at the mercy of customer support tickets. As one player put it: *"If they are not able at all to recovery our lost items, they need to rethink the compensation."*

---

## 🍓 STRAWBERRY MILKSHAKE: The "Poor Man's Pestilence"

With the tooltip finally fixed to show the accurate numbers (2.40% Weapon Damage DoT over 4.50 seconds), the community math guys got to work. The verdict? It's bad.

Players are reporting the Anarchy DoT ticking for a dismal ~38 damage per tick. The gun has officially been dubbed the "poor man's Pestilence" across the subreddit. 

**The Playbook:** Do not spend your hard-earned recalibration materials on this weapon yet. Ubisoft has already stated in the 1.2.1 notes that a buff is under review. Farm your Weekly Mission Exotic Manuals, stockpile the materials, and wait for the green light.

---

## 📢 THE OFFICIAL QUIET

Aside from the inventory fix, official communications have been light. Over the last 48 hours, the official `@TDResurgence` account has made exactly two community comments:
1. Confirming they love a "Fashion Friday" idea.
2. Confirming the **DZ Tactical Augmentation reward bug** is still broken and under investigation with no ETA.

Based on their historic post cadence, they average one major communication every 5 to 10 days. We expect the next major roadmap or patch update in **late May or early June**.

---

## 🎯 WHAT THE COMMUNITY IS ACTUALLY GRINDING

If you look past the bug reports, the endgame players are hyper-focused on three things right now:

### 1. Cracking the Aegis / Tech Operator Code
Aegis is the dominant [specialization](/guides/which-specialization-to-pick/) discussion. Players are realizing that to make the drone DPS viable in Legendary content, you need to absolutely max out Engineering and Skill Multi-Shot. 
*   **The Math:** Skill Multi-Shot actually scales the 3-second Aegis burst into multiple damage instances. 
*   **The Reality:** Even with optimized gear, Striker drones pop instantly in Legendary content to molotovs and sniper fire. Many are pivoting back to Tactical Pharma for reliable AoE and survivability.

### 2. The Gold OS Protocol Drought
The RNG on Gold OS Protocols is breaking spirits. Regulars are reporting streaks of purple drops even from weekly bounties. Save your Phoenix Credits for augmentations; Gold OS drops remain purely tied to weekly bounty luck.

### 3. Crafting vs. Drops
Maxing your crafting bench to level 10 gives a 25% chance at gold gear, but the 2-hour wait times and heavy RNG on talents mean world drops are still king. Use your crafting bench for SHD leveling, not god-roll hunting.

---

## 🩹 STILL BROKEN (Ongoing Issues)

Don't adjust your routers; these are confirmed live issues as of May 23:
*   **DZ Leaders Not Dropping Loot:** Reports of boss kills resulting in zero drops (no chipsets, no equipment).
*   **Asia & PC Connection Issues:** Persistent disconnects since the Phase 2 update. See the [error codes guide](/guides/division-resurgence-error-codes/) for troubleshooting steps.
*   **Aegis Ultimate Recast Bug:** If your first Aegis ultimate shield is still active, popping a second ultimate fails to fire. 
*   **Civilians in Need:** The Div 1 carryover mechanic for giving away bottled water is coded so rarely you have a better chance of finding a Gold OS Protocol.

---

## 💬 THE PVP META FRUSTRATION CYCLE

The Dark Zone remains a terrifying place. The current complaints revolve entirely around **Bulwark Tanks** and the **Double-Barrel Shotgun**. 

When a player with 150k health and 5,000 armor can still be two-tapped by a shotgunner, or when a Bulwark shield absorbs an entire magazine without flinching, the PvP balance is going to be called into question. Expect Bulwark to be the loudest complaint until Phase 3. 

---

## 🗓️ COMING UP: DERELICT STORE SPEEDRUN

Take a breath, because next week the grind continues. 
*   **OW Nest Speedrun (Derelict Store):** Starts May 29 through June 2. 
*   **Phase 3:** Drops in June, bringing the new Scorpio [exotic](/database/exotic-weapons/), the Freemen apparel set, and the Boombox weapon skin. 

Patch 1.2.1 and the May 21 server patch were the dev team's response to the roughest week of the game's life. The bleeding has stopped, but the community is still licking its wounds. **Phase 3 is the real next test.** See you in the Dark Zone.
