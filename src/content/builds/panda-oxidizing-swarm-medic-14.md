---
title: "Field Medic — The Toxic Waste Dump"
specialization: "Field Medic"
playstyle: "Hybrid"
pve: true
pvp: true
lastUpdated: 2026-05-23
patch: "Season 2"
tags: ["division-resurgence", "panda", "oxidizing-swarm", "field-medic", "pvp-build", "pve-build", "dark-zone", "skill-damage"]
description: "Panda's terrifying all-around Oxidizing Swarm build that deletes players in the Dark Zone before they even know what hit them."
author: "RapidF5 (via Panda)"
featuredImage: "/images/division-resurgence-panda-oxidizing-swarm-build.png"
imageAlt: "Division Resurgence Panda Oxidizing Swarm Field Medic Build"
shareTitle: "The All-Around Oxy Nuke Build"
shareText: "Delete squads in the Dark Zone with Panda's Oxidizing Swarm build. Glass Cannon meets infinite hive duration."
faqs:
  - question: "Why run Glass Cannon if it makes me squishy in PvP?"
    answer: "Because you're playing with a team, not trying to be a one-man army. Plus, an extra 28% damage means they usually melt into a puddle of green goo before they can even aim their gun at you. Just hide behind your Vanguard like a good little medic and watch the kill feed light up."
  - question: "Why isn't my Surgical Engineering OS proccing on the hive?"
    answer: "Because the game hates you. Jokes aside, the hive doesn't count as a 'deployable' for Surgical Engineering's proc condition, even if you throw it at someone's head. That's why we use Tactical Layering instead. Stop trying to force it."
  - question: "Is the Armored Strengthening chip broken?"
    answer: "Probably. Panda has filed three tickets about the armor buff not appearing on the HUD, and the devs have left him on read. We're running it anyway because we live in hope, but keep an eye on it."
---

## What It Is

> 🎬 **Build credit:** This build comes straight from the mind of [**Panda**](https://www.twitch.tv/panda). Watch the breakdown here: [BEST ALL-AROUND BUILD in The Division Resurgence (PVE + PvP)](https://www.youtube.com/watch?v=CB3Rq2VIP0g). Go drop him a follow!

Welcome to the **"Toxic Waste Dump"** Medic. If you thought Field Medics were just there to awkwardly throw healing hives while the Vanguard does all the actual fighting, you haven't seen Panda's Oxidizing Swarm build. This archetype is an Engineering-heavy "Oxy" build that treats the Geneva Convention like a coloring book and turns team fights into a localized hazmat emergency. 

Whether you're running PvE, sweating in Conflict, or stealing loot in the Dark Zone, this build is designed to nuke players almost instantly. You'll be dropping Oxidizing Swarm hives that last nearly a full minute, melting entire squads while you sit back and casually reload your Vector.

## The Stat Breakdown

Panda showed off some absolutely unhinged stats on stream. 

| Stat | Value | Why It's Terrifying |
|---|---|---|
| **Firepower** | 598 | Who needs guns when you have acid? |
| **Toughness** | 380 | You are made of glass, but they are made of meat. |
| **Engineering** | 1,700 (1.7k) | This is where the magic happens. |
| **DPS** | ~730,000 solo | Bumps up to **~800,000** when grouping with allies thanks to the backpack talent. |
| **Armor** | ~10,000 | Enough to survive a stiff breeze. |
| **Health** | ~130,000 | You'll want to avoid taking direct hits. |

*Note: In a team, the Oxidizing Swarm hive lasts roughly 45–50 seconds, while Chem Traps sit around 25–30 seconds. That's an eternity in a PvP firefight.*

## The Statistical Engine

| Slot | Brand/Set | Talent | Base Stat | Attribute 1 | Attribute 2 |
|---|---|---|---|---|---|
| Mask | Jackpot | — | Engineering | Max Crit Chance | Max Multi-Shot Chance |
| Gloves | Boom Boom | — | Engineering | Skill Crit Chance | Skill Multi-Shot |
| Holster | Jackpot (T2) | — | Engineering | Max Crit Damage | Max Multi-Shot |
| Kneepads | Jackpot | — | Engineering | Crit Chance | Crit Damage |
| Chest | *Unnamed T1* | **Glass Cannon 2** | Engineering | Skill Multi-Shot Chance | Skill Cooldown Recovery |
| Backpack | Captain Cow (T1) | **Brainstorm 2** | Engineering | Skill Intensity | Skill Crit Damage |

- **Primary:** Lightweight M4 — In Sync + Fully Charged *(Rolled for Weapon Multi-Shot + Weapon Crit Damage. Panda wants to re-roll In Sync for more Crit Damage, but material drop rates are miserable.)*
- **Secondary:** First Wave Vector — Precise Trauma 2 + Tactical Reload *(This is the gun he uses for literally everything. Rolled for Skill Piercing + Skill Multi-Shot.)*
- **OS Protocol:** Tactical Layering
- **Skills:** Oxidizing Swarm (Hereafter Resilience) · Chem Traps (Armored Strengthening) · Skill Chip #3 (Enhanced Formula)
- **Signature:** Oxidizing Swarm
- **Gear Mods:** Mostly gold Engineering mods across all pieces, with a couple of gold Armor mods mixed in.

**Jackpot 4-Piece:**
- **2pc:** Skill Radius +12.00%
- **3pc:** Skill Critical Chance +10.80%
- **4pc:** Skill Critical Damage +24.00%
*(Note: Panda intentionally isn't running a secondary 2-piece set yet, but the long-term goal is to slot in a 2-piece Captain Cow for the movement speed buff.)*

**Why These Talents**

**Glass Cannon 2 (Chest):** +28% extra damage is just too nice. Yes, it makes you squishy, but you offset it by playing behind your team and stacking damage reduction elsewhere. Kill them before they kill you.

**Brainstorm 2 (Backpack):** Grants +40% skill duration when allies are within 20m. This is the secret sauce. This is why the Oxidizing Swarm lasts for nearly 50 seconds. Panda specifically tested this against Minion Training and Prime Engineer, and Brainstorm 2 won because keeping the swarm active is the entire point of the build.


## The Skills

### Oxidizing Swarm (Hive) — Main Damage
This is your bread and butter. You will drop this hive 90% of the time.
- **Mod Priorities:** Intensity > Cooldown Reduction > Critical Hit Damage > Weapon Damage > Critical Hit Chance > Multi-Shot
- **Firmware Chip:** **Hereafter Resilience** (If you're old, this used to be called Acidic Effect).

### Chem Traps
- **Mod Priorities:** Intensity > Skill Duration > Damage Reduction > Trap Radius > Critical Hit Chance
- **Firmware Chip:** **Armored Strengthening**
- *Warning:* Panda suspects this chip is currently bugged and not applying the armor boost. He's filed tickets, but until it's fixed, consider it a placebo.

### Skill Chip #3 — Enhanced Formula
- *Formerly known as Feebleness.*
- Currently stuck with a Radius roll, but you want to chase **Crit Chance, Skill Cooldown Recovery, Cooldown Reduction, or Radius.**

## OS Protocol: Tactical Layering

Panda considers **Tactical Layering** the absolute #1 OS for this build. 

**What it does:** Tactical Layering's talent is: *Deploying a Skill grants +15% Engineering, up to 3 stacks.* 

**Why it's so powerful here:** Even though the game doesn't count dropping your Hive as a "deployment" to trigger this buff (a frustrating quirk Panda tested extensively), the build runs **Chem Traps**. Every time you toss a Chem Trap, you gain a massive +15% Engineering buff. You can easily keep this stacked up to 3 times (+45% Engineering!) just by actively using your Chem Traps, which supercharges the damage of your Oxidizing Swarm while it sits on the field.

*What about Surgical Engineering?* Surgical Engineering is the runner-up (granting Skill Crit Damage on skill crits), but the raw, scaling Engineering power of Tactical Layering simply outpaces it when you're actively managing your traps.

## Stat Priorities Summary

If you're confused about what to roll, follow this hierarchy:
1. **Skill Intensity** (Your main scaler, since Brainstorm 2 handles duration).
2. **Skill Critical Hit Chance** (Stack it everywhere).
3. **Skill Multi-Shot** (Stack it everywhere else).
4. **Skill Critical Hit Damage**
5. **Skill Cooldown Recovery / Reduction**
6. **Skill Piercing Damage** (Vector specific).
7. **Damage Reduction** (To stop Glass Cannon from getting you instantly deleted).

## The Verdict

In Panda's own words:
> *"This build is excellent for pretty much anything. You can solo content with it. It's perfect for team fights. It's perfect for Conflict. It's perfect for DZ, especially in a group… This thing hits super hard and nukes a lot of players in the DZ almost instantly."*

Get your squad, equip Glass Cannon, and start melting.
