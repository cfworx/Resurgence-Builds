---
title: "The Pocket Medic Your Team Will Kill For"
...
# The Pocket Medic Your Team Will Kill For

## Your Team Will Fight Over You
...
A **Field Medic** build for The Division: Resurgence that runs three independent healing engines all at the same time, every fight, with zero effort. Your team literally cannot die unless they sprint off a cliff. Tested and verified, math included.
...
## How This Build Makes Your Team Immortal

Legendary missions punish two things: teams that die, and healers who can't keep up. This build fixes both problems at once by running **three healing engines** simultaneously. None of them depend on a proc chain. None of them require you to think. You press your buttons, aim for heads, and your team's HP bars stop moving.
...
### Engine 1 - Raw Heal Output (you're the printer, HP is the ink)
...
### Engine 2 - Team Heal Multiplier (making your heals hit different)
...
Three independent team-side multipliers, all stacking on top of Engine 1. Your allies don't need to do anything. They just... stop dying.

### Engine 3 - Skill Uptime (your station outlives the mission)
...
## One Gun. Two Healing Talents. Zero Swapping.

You run a single primary with both healing talents on the same weapon. Point at heads, pull trigger, and every bullet reloads your team's heal multipliers. **No mid-fight swapping, no ability management, no cover dancing.** Just shoot things in the face and your team lives.
...
|---|---|---|
| +20% Healing Intensity | Captain Cow 4pc | x 1.20 |
| +18% Healing Intensity | Medical Accuracy (headshot up) | x 1.18 |
| +20% Healing Intensity | Medical Excellence OS passive | x 1.20 |
| +9.6% Skill Intensity | Mutual Support (Hive deployed, ally <20m) | x 1.096 |
| **Output subtotal** | 8,000 x 1.20 x 1.18 x 1.20 x 1.096 | **~ 14,908 HP** |

### Target stack (what the receiving ally multiplies)

| Multiplier | Source | Effect on heal |
|---|---|---|
| +21% Incoming Healing | Slaughter Healing (3 stacks) | x 1.21 |
| +40% Incoming Healing | Effective Triage (ally <50%) | x 1.40 |
| +14.4% Received Healing | Healing Elites 2pc | x 1.144 |
| **Target subtotal** (Triage + Slaughter) | 14,908 x 1.21 x 1.40 | **~ 25,254 HP per tick** |
| + Medical Excellence target-pop (<50% ally) | +12.5% Max HP added to target | **+ 12,500 HP** |
| **Total per tick on a low-HP ally** | All multipliers + ME target-pop | **~ 37,754 HP** |
| + Medical Excellence team-pop (>50% ally) | +5% Max HP fires on every ally | +5,000 HP x 4 allies = **+20,000 HP** |
...
| Per-piece Skill Duration rolls (~4 pieces x ~4%) | ~+16% |
...
> **~37,754 HP per tick x ~56 ticks = ~2.11 million HP per Support Station deployment** if a single ally were locked at <50% inside the radius for the full duration.
...
- **Per cast: ~8 extra Support Station ticks** (from ~48 to ~56) x ~37,754 HP per tick = **~302,000 additional HP of healing per Support Station deployment** compared to a chest with no duration bonus.
...
| Brainstorm (Roamer Pack) | +30% Skill Duration -> longer Support Station + Hive uptime | Passive, always on in team build |
...
|---|---|---|
| **Deep Focus** | +28% Skill Duration, -10% Skill Radius | **YES - indirectly.** Extends Support Station / Hive uptime. Only chest talent that boosts a team-facing effect with no condition. |
| Territory Expansion | +30% Skill Radius while at full Health | INDIRECT - widens Support Station coverage. Trigger conditional on full HP, fragile. |
...
|---|---|---|
| **Captain Cow** | +3.6% MS 2pc / +14.4% Skill Duration 3pc / **+20% Healing Intensity 4pc** | **Best pre-elite bridge.** +20% HI 4pc is the highest flat HI bonus on any gear set - beats Healing Elites for raw output. Run from launch through endgame elite unlock. |
| **Healing Elites** | +14.4% Received Healing 2pc / +15% Release Extra Protection 3pc / +28% Armor 4pc | **Endgame target** - requires elite missions. Trades Captain Cow's +20% HI 4pc for the Release Extra Protection overshield, which is the team-wide piece that locks in upstream damage reduction. |
| Boom-Shakalaka | -6% Skill CDR / +18% Skill Radius / +12% Skill Intensity 4pc | Throughput / uptime hybrid. Skill Radius widens Support Station (offsets Deep Focus -10% trade). |
...
| **Effective Triage Support Station mod** (x6 = full set) | Support Station multiplier. 3 to activate set bonus; chase 6 total for swaps. | Skill Health / Healing Intensity rolls |
| **Mutual Support Hive mod** (x6 = full set) | Hive SI multiplier. The exact name is Mutual Support - not "Coordinated Strike." | Skill Health / Healing Intensity rolls |
...
|---|---|
| Auto Trauma Treatment Support Station mods | Bench set - keep 3 rolled for chaotic runs where auto-revive matters more than +40% IH spike. |
| Buddy Treatment Hive mods | Bench set - alternative to Mutual Support. +2% IH passive + auto-drones to allies in 15m. Trade SI scaling for passive proximity heal. |
| Doom Dealer / Beneficial Association / Emergency Measures Swarm mods | Healing Swarm is a signature, not a combat skill, so mod sets do not apply at all. Keep none. (v8 correction: Doom Dealer is a real Swarm mod - still not useful here.) |
...
| Slaughter Fortitude / Conceited SMG | Secondary stat-stick. Holster passives only - never drawn. |
