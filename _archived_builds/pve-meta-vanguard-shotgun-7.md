---
title: "The Alt-F4 Shotgun Button - Vanguard PvE Shotgun Build Guide (DPS)"
specialization: Vanguard
playstyle: DPS
pve: true
pvp: false
lastUpdated: 2026-05-17
patch: "Season 2"
tags: ["commando", "shotgun", "phalanx-attack"]
description: "The premier PVE DPS build. A shotgun setup so powerful it functions as an alt-F4 button for elites."
author: "RapidF5 (via Godlike)"
featuredImage: "/images/division-resurgence-vanguard-double-barrel-shotgun-pve-build.webp"
imageAlt: "Division Resurgence Vanguard Double Barrel Shotgun PVE Build"
shareTitle: "Share the Alt-F4 engine"
shareText: "Tweet this so the devs see it and nerf us into the ground next patch."
plannerHash: "s=Vanguard&sc=Commando&n=Alt-F4+Shotgun&ms=gs-3&m1=Weapon+Critical+Hit+Chance&m2=Headshot+Damage&cs=gs-5&ct=ba-7&c1=Weapon+Damage&c2=Weapon+Critical+Hit+Damage&ps=gs-5&pt=bp-24&p1=Weapon+Critical+Hit+Damage&p2=Weapon+Multi-Shot+Chance&gs=gs-3&g1=Weapon+Critical+Hit+Damage&g2=Weapon+Critical+Hit+Chance&hs=gs-3&h1=Weapon+Critical+Hit+Chance&h2=Weapon+Damage&ks=gs-3&k1=Weapon+Multi-Shot+Chance&k2=Weapon+Critical+Hit+Damage&w1s=sw-22&w1tr=T2&w1t=wt-54&w1t2=wt-65&w2s=sw-6&w2tr=T2&os=os-29"
faqs:
  - question: "Is the shotgun Vanguard the best PvE build in Division Resurgence?"
    answer: "For pure 'delete everything in front of you' energy, nothing else comes close. The burst damage on this build makes Legendary elites look like tutorial enemies. The tradeoff is range. You have to be close enough to smell the enemy's cologne, which means you're also close enough to get slapped. If you can manage the distance game, this is the undisputed PvE king."
  - question: "Why shotgun instead of an assault rifle on Vanguard?"
    answer: "Because assault rifles spread their damage over 30 bullets and this shotgun concentrates it into one shell that removes an enemy from the census. The DPS-per-second number might look similar, but the DPS-per-trigger-pull is an order of magnitude higher. In practice, this means enemies die before they can shoot back, which is the best form of damage mitigation in the game."
  - question: "Can I use this Vanguard shotgun build in PvP?"
    answer: "In the Dark Zone at close range, this build is a horror movie for the person on the other end. The problem is closing the gap against players who can move and think. Against NPCs who walk predictably into your kill zone, it's easy. Against a player strafing with Lady Death, you need to land your shots or you're dead in the reload window. High risk, higher reward, highest comedy when it works."
  - question: "What shotgun should I use for this Division Resurgence build?"
    answer: "Any shotgun with high per-pellet damage and the right talent roll. The specific weapon name matters less than the talent synergy. Look for Torrential Downpour or any talent that stacks damage per hit. The shotgun is a delivery mechanism for talents. The talents are the actual build. If your shotgun has bad talents, it's just a loud noise maker."
  - question: "Is Vanguard the best specialization in Division Resurgence?"
    answer: "For solo play and PvP, it's the best by a comfortable margin. For group PvE support, Field Medic and the new Vanguard Scout Recon are more impactful. Vanguard's dominance is mostly about the Scanning Pulse being legal wallhacks and the mobility skills being better than everyone else's. Ubisoft knows this. They've known this since launch. The balance patch is coming any day now. (We've been saying that for months.)"
---

## What It Is

Do you look at an enemy elite's health bar and think, "I want that to go away right now"? Welcome to the delete button. This Vanguard PvE shotgun build guide is tuned for high-difficulty DPS, letting you mathematically erase targets at close range. It's the perfect aggressive setup for when you want to speed-run existence and make the servers beg for mercy.

This is the current best PVE DPS build in the game. Period. It uses the **Vanguard spec**, **4pc Phalanx Attack + 2pc Gunny Johnny**, a **Double Barrel Shotgun**, and the **Explosive Round OS**. Made famous by Godlike, who calls it "the premier DPS build in the game when it comes to firearms."

You are going to walk up to things and mathematically erase them from the server.

**Build Overview**

- **Specialization:** Vanguard (Commando)
- **Archetype:** Close-range burst DPS / one-shot deleter
- **Difficulty Ceiling:** Comfortable through Challenging with a Bulwark babysitter. Solo Legendary is technically possible if your reflexes haven't been legally declared deceased.
- **Core Loop:** Pulse the pack → drop Smart Cover → close to ≤15m → aimed shotgun crit → Explosive Round AOE detonation + Multi-Shot procs → everything in a 5m radius files a missing persons report.

## Why This Is The Uninstall Button

Every meta build in Division Resurgence tries to optimize sustained DPS , steady damage over time, manage your cooldowns, keep the rotation flowing. This build doesn't have a rotation. It has a **trigger pull**.

Phalanx Attack is the *only* gear set that rolls Magazine Size, Rate of Fire, AND Multi-Shot at 2/3/4pc. Stack that absurd combination on a weapon that already fires two shells per pull. Layer Pulse Vulnerability on top. Add an OS that triggers a 5m AOE explosion on every aimed crit for **2,000% Firepower damage**. The result is a build where each trigger pull is doing 3-4 separate, massive damage instances on every enemy in the pack.

**The Catch:** You are extremely squishy. You have pure Firepower cores and terrible Toughness. If you don't have a Bulwark to pull aggro, or if you mess up your Smart Cover positioning, you will die faster than the things you are trying to kill. High risk, hilarious reward.

## Weapons

**Primary , Double Barrel Shotgun (T2 Shotgun)**

**Talent: Steady Reload.** Reloading grants +18% Weapon Damage for 5 seconds, up to 5 stacks. The Double Barrel reloads constantly because it holds two shells. Every reload is a damage ramp. You're not reloading , you're winding up a catapult.

**Talent 2: Thick Skin.** Bonus survivability to offset the glass-cannon stat line. You're standing at shotgun range with zero Toughness investment. Every little bit helps keep you vertical long enough to pull the trigger one more time.

**Secondary , FAMAS (T2 AR)**

The cleanup weapon. When the Double Barrel deletes everything in the immediate vicinity and a few stragglers are limping away at 10% HP, you don't reload the shotgun for them. You swap to the FAMAS and spray until the room is quiet. It's the broom after the wrecking ball.

## The Build

| Slot | Set | Talent | Core | Bonus 1 | Bonus 2 |
|---|---|---|---|---|---|
| Mask | Phalanx Attack | , | Firepower | Weapon Crit Chance | Headshot Damage |
| Gloves | Phalanx Attack | , | Firepower | Weapon Crit Damage | Weapon Crit Chance |
| Holster | Phalanx Attack | , | Firepower | Weapon Crit Chance | Weapon Damage |
| Kneepads | Phalanx Attack | , | Firepower | Multi-Shot | Weapon Crit Damage |
| Chest | Gunny Johnny | **Duality** | Firepower | Weapon Damage | Weapon Crit Damage |
| Backpack | Gunny Johnny | **Street Fighter** | Firepower | Weapon Crit Damage | Multi-Shot |

- **Primary:** Double Barrel Shotgun (T2) , Steady Reload + Thick Skin
- **Secondary:** FAMAS (T2 AR)
- **OS Protocol:** Explosive Round (Firepower, High-End)
- **Skills:** Scanning Pulse (Optimal Awareness 3pc) + Commando Smart Cover (Armored Train 2pc)
- **Signature:** Tactical Link
- **Gear Mods:** 12× Firepower (Glass Cannon supremacy)

**Phalanx Attack 4-Piece Bonuses**

- **2-piece:** Magazine Size +18%
- **3-piece:** Rate of Fire +18%
- **4-piece:** Multi-Shot Chance +12%

**Chest & Backpack Justification**

**Duality** on the chest doubles down on the aggressive playstyle , rewarding you for staying in the fight and capitalizing on the burst window. The Gunny Johnny 2pc brand bonus stacks with your Firepower core, and Duality synergizes with the "all-in on one trigger pull" philosophy. You're not playing safe. You're playing fast.

**Street Fighter** on the backpack is the close-range amplifier. Since you're already standing at ≤15m to fire the shotgun, Street Fighter's bonus activates naturally. The Multi-Shot roll on the backpack stacks with Phalanx 4pc, and the talent reward for close-range combat means you're being paid for doing what you were already going to do , stand uncomfortably close to dangerous things.

## Skill Loadout

**Scanning Pulse , Optimal Awareness (3pc)**

The Scanning Pulse is your pre-engagement opener. It applies **+5% Vulnerability** to every pulsed target and grants you **+25% Weapon Damage** while active. The Commando subspec layers **+8% Crit Chance per pulsed target within 70m** on top. Optimal Awareness 3pc extends the Pulse duration and tightens the stat window, ensuring you get maximum uptime during your burst window.

**Commando Smart Cover , Armored Train (2pc)**

Your only survival mechanic. Smart Cover provides a Damage Reduction layer that keeps you alive at shotgun range. Armored Train 2pc adds extra Toughness while in cover. Without Smart Cover active, you are a tissue paper strongman , impressive on the way in, confetti on the way out. Drop it behind you before engaging and don't leave it.

**Signature , Tactical Link**

The kill button's kill button. +20% Crit Chance and a free target lock for ~22 seconds. Pop this before your first aimed shotgun crit and watch every pellet trigger Explosive Round detonations. This is your "everything dies now" window. Use it on elite packs and bosses. Do not waste it on red-bar trash.

## How It Works (The Math)

Three damage engines stack on *every single shotgun trigger pull*:

**Pulse Vulnerability Stack**

| Component | Bonus |
|---|---|
| Scanning Pulse active | +25% Weapon Damage |
| Pulse Vulnerability debuff | +5% Vulnerability on pulsed targets |
| Commando subspec | Up to +8% Crit Chance per pulsed target within 70m |

**Multi-Shot Probability**

| Source | Multi-Shot Chance |
|---|---|
| Phalanx Attack 4pc | +12% |
| Street Fighter (close range) | Additional proc chance |
| **Double Barrel (2 shells per pull)** | Each shell rolls independently |

Since the Double Barrel fires two shells per pull, multi-shot procs feel like an extra full pellet group hitting them in the chest. Each shell independently rolls for Multi-Shot, meaning the effective multi-shot rate per trigger pull is significantly higher than the listed 12%.

**Explosive Round OS Detonation**

| Condition | Effect |
|---|---|
| Aimed critical hit | 5m AOE for 2,000% Firepower damage |
| Multiple crits per pull | Multiple simultaneous detonations |
| With Tactical Link (+20% CHC) | Near-guaranteed detonation per pull |

**Full Damage Stack Summary**

| Source | Bonus |
|---|---|
| Scanning Pulse | +25% Weapon Damage |
| Pulse Vulnerability | +5% on pulsed targets |
| Phalanx Attack 4pc | +12% Multi-Shot Chance |
| Explosive Round OS | 2,000% Firepower AOE per aimed crit |
| Tactical Link | +20% Crit Chance for ~22s |
| Duality chest | Close-range damage amplifier |
| Street Fighter backpack | Close-range talent bonus |
| Steady Reload | +18% Weapon Damage per reload stack (up to 5) |
| Gunny Johnny 2pc | Brand bonus stacking with Firepower |

**The Synergy Chain**

Pulse debuffs targets → you close to ≤15m → Smart Cover provides DR → aimed shotgun crit → Explosive Round detonates 5m AOE → Multi-Shot fires extra shells → each extra shell can independently crit → each crit can independently detonate → Tactical Link ensures near-100% crit rate → everything in the 5m blast radius gets hit by multiple overlapping detonations → the server briefly questions its life choices.

## OS Justification

**Explosive Round > Dominating Presence** for this build. Dominating Presence gives +5% Weapon Multi-Shot Chance per enemy within 20m (cap +25%), which stacks additively with Phalanx 4pc's +12% for a total of +37% Multi-Shot. That's nice.

Explosive Round gives a **2,000% Firepower AOE on every aimed crit**. The Double Barrel fires two shells per pull, each of which can crit independently, each of which can proc Multi-Shot for additional shells, and *each of those* can crit and trigger another detonation. You're not comparing +25% Multi-Shot to an AOE nuke , you're comparing a nice bonus to a chain reaction.

One is a percentage increase. The other is a physics engine stress test. Run Explosive Round.

## Gear Ladder

| Tier | Set | Notes |
|---|---|---|
| T0 Bridge | Phalanx Attack 4pc + any brand 2pc | Phalanx drops from T0+. Your 4pc bonus is online from day one. Use whatever brand chest/backpack you find with decent Firepower rolls. |
| T1 Step | Phalanx Attack T1 pieces + Gunny Johnny 2pc | Upgrade Phalanx pieces to T1 stat rolls. Start hunting Gunny Johnny chest and backpack for Duality and Street Fighter. |
| T2 Destination | Phalanx Attack T2 4pc + Gunny Johnny T2 2pc | Full T2 stat rolls on everything. Recalibrate for max WCHC/WCHD. The build hits final form here and elites start filing insurance claims. |

## Combat Rotation

1. **Pulse the pack.** Identify your victims. Scanning Pulse applies +5% Vulnerability and feeds Commando's +8% CHC per pulsed target.
2. **Drop Smart Cover behind you.** This is your Damage Reduction layer. Without it, you are dead. Not "low HP" dead. Loading-screen dead.
3. **Wait for your Bulwark to engage and pull aggro.** Be patient. You are not the opening act. You are the headliner.
4. **Close the distance to shotgun range (≤15m).** Street Fighter activates. Your damage window is open.
5. **Pop Tactical Link.** +20% Crit Chance and target lock. The ~22-second window of absolute godhood begins.
6. **Aimed shotgun crits trigger the Explosive Round AOE.** Pull the trigger until there is nothing left. Each pull fires two shells. Each shell can crit. Each crit detonates. The math is on your side.
7. **Swap to the FAMAS to clean up the stragglers** who are trying to run away. They won't get far. They never do.

## Loot Quick-Reference

| Item | Where to Look |
|---|---|
| Phalanx Attack 4pc | T0+ gear drops , available from first content, farmable everywhere |
| Gunny Johnny 2pc (Chest + Backpack) | T1+ brand drops , world events, mission rewards |
| Double Barrel Shotgun (T2) | T2 Shotgun drops , world events, elite missions, DZ |
| FAMAS (T2 AR) | T2 AR drops , world events, mission completions |
| Explosive Round OS | HE Firepower OS , high-difficulty mission drops |
| Duality chest talent | Recalibrate any Gunny Johnny chest |
| Street Fighter backpack talent | Recalibrate any Gunny Johnny backpack |
| Optimal Awareness skill mods | Vanguard skill mod drops , skill mod caches |
| Armored Train skill mods | Vanguard skill mod drops , skill mod caches |

## TL;DR - Five Things

1. **The Double Barrel fires two shells per pull** , each one independently rolls for crit and Multi-Shot. Basically a slot machine that only pays out in violence.
2. **Explosive Round OS detonates a 5m AOE on every aimed crit** for 2,000% Firepower damage. Multiple crits per pull = multiple simultaneous war crimes.
3. **Phalanx Attack 4pc is the only set** that gives Mag Size, Rate of Fire, AND Multi-Shot in one package. It was made for this.
4. **Tactical Link's +20% Crit Chance window** is where the build goes from "really good" to "someone clip that."
5. **You will die constantly if you don't have a Bulwark friend.** This build is a glass cannon in the most literal, shattering-on-impact sense possible.

## Related Builds

- **Take it to standard PvP:** Check out our [Conflict PvP Vanguard Warlord Build](/builds/conflict-pvp-fearless-warlord) for arena dominance.
- **Ruin lives in the Dark Zone:** See our [Vanguard Dark Zone PvP Lady Death Build](/builds/dark-zone-pvp-vanguard-lady-death-3) for toxic mobility.
- **Need a pocket healer?** Bring along a friend running the [Legendary Healer Field Medic Build](/builds/legendary-healer-field-medic-5).
- **Want to sit in cover instead?** Try the [Cover Shooter Sniper Build](/builds/cover-shooter-sniper-2) and let someone else take the hits.
